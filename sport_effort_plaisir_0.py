            ...
            if t not in used_values:
                ...
                if r not in used_values:
                    ...
                    if i not in used_values:
                        used_values.add(i)
                        for f in range(2, 7):
                            if f not in used_values:
                                used_values.add(f)
                                for o in range(2, 7):
                                    if o not in used_values:
                                        used_values.add(o)
                                        s = (2 * o + (2 * r) // 10) % 10
                                        if s not in used_values:
                                            used_values.add(s)
                                            for a in range(2, 6):
                                                if a not in used_values:
                                                    used_values.add(a)

                                                    numbers_tested += 1
                                                    sport = t + 10 * r + 100 * o + 1000 * p + 10000 * s
                                                    effort = t + 10 * r + 100 * o + 11000 * f + 100000 * e
                                                    plaisir = r + 10 * i + 100 * s + 1000 * a + 10000 * l + 100000 * p

                                                    if sport + effort == plaisir:
                                                        solutions.append((sport, effort, plaisir))

                                                    used_values.remove(a)
                                            used_values.remove(s)
                                        used_values.remove(o)
                                used_values.remove(f)
                        used_values.remove(i)
                    used_values.remove(r)
                used_values.remove(t)
